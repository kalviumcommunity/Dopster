    ...
    if (state) {
        ...
        <div id="nav-right">
          <div className="menu">
            <div id="ul">
              ...
              <Link to="/contactus">
                {" "}
                <h5 className="nav-items">Contact</h5>
              </Link>
              <Link to="/myprofile">
                {" "}
                <h5 className="nav-items">My Profile</h5>
              </Link>
            </div>
          </div>
          <div id="signup__div-nav">
            <button
              id="signup__button_nav"
              onClick={() => {
                localStorage.clear();
                dispatch({ type: "CLEAR" });
                navigate("/");
      ...
      return [
        <div id="nav-right">
          ...
          <div id="signup__div-nav">
            <button onClick={() => navigate("/signup")} id="signup__button_nav">
              {" "}
              Signup
            </button>
          </div>
    ...
    <>
     ...
     <button onClick={()=>navigate("/signup")} id='signup__button_nav'>  Signup</button>
        ...
        <div className="hamburger-menu">
          <a id="hamburger" href="#" onClick={() => navfunc()}>
            <GiHamburgerMenu />
          </a>
        </div>
        <div id="mobile-menu">{renderMobileList()}</div>
      </nav>
    </>
  );
}
